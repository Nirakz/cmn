    ...
    socket.on("user-is-not-typing", (data) => {
      if (data.typingGroup) {
        ...
        if (clients[data.groupId]) {
          emitNotifyToArray(clients, data.groupId, io, "response-user-is-not-typing", responseToGroup);
        }
      } else {
        let responseToUser = {
          currentUserId: socket.request.user._id
        ...
        if (clients[data.contactId]) {
          emitNotifyToArray(clients, data.contactId, io, "response-user-is-not-typing", responseToUser);
        }
      }
    });
